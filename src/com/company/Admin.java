...
//                    Connection connection = Database();  //Connect to the database
                    try {
                        ...
                        ResultSet resultSet = stmt.executeQuery("SELECT * FROM librarian WHERE lib_name='"+ username+"'"); //Execute query
                        String pass = null;
                        while(resultSet.next()){
                            pass = resultSet.getString("lib_pass");
                        }
                        if (!pass.equals(password)) { //Move pointer below
                            error("Wrong Username/Password!"); //Display Message
                        } else {
                            f.dispose();
                            librarian.library_options();
                        }
                    } catch (Exception ex) {
                        error("Some error");
